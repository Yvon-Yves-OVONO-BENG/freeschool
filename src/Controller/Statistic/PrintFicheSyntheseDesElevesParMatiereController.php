...
#[Route("/statistic")]
class PrintFicheSyntheseDesElevesParMatiereController extends AbstractController
{
    ...
    #[Route("/printFicheSyntheseDesElevesParMatiere/{resume<[0-1]{1}>}", name:"statistic_printFicheSyntheseDesElevesParMatiere")]
    public function printFicheSyntheseDesElevesParMatiere(Request $request, int $resume = 0): Response
    {
        ...
        $pdf = $this->statisticService->ficheDeSyntheseDePerformanceDesElevesParMatiere($classroomStatisticSlipPerSubjects, $firstPeriodLetter, $idP, $school, $schoolYear, $resume);
        ...
        switch ($firstPeriodLetter) 
        {
            case 's':
                ...
                {
                    return new Response($pdf->Output(utf8_decode("STUDENT PERFORMANCE SUMMARY SHEET BY SUBJECT ".$idP ), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                } else {
                    return new Response($pdf->Output(utf8_decode("FICHE SYNTHESE DE PERFORMANCE DES ELEVES PAR MATIERE ".$idP), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                }
                break;
            ...
            case 't':
                if ($subSystem->getId() == 1 ) 
                {
                    return new Response($pdf->Output(utf8_decode("STUDENT PERFORMANCE SUMMARY SHEET BY SUBJECT of term ".$idP ), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                } else {
                    return new Response($pdf->Output(utf8_decode("FICHE SYNTHESE DE PERFORMANCE DES ELEVES PAR MATIERE DU TRIMESTRE ".$idP), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                }
                break;
            ...
            default:
                if ($subSystem->getId() == 1 ) 
                {
                    return new Response($pdf->Output(utf8_decode("Annual slip per subject "), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                } else {
                    return new Response($pdf->Output(utf8_decode("Statistiques annuel par matière"), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                }
                break;
        }
    }
