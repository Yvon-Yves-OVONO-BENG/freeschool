...
class ReportService 
{
        ...
        if(!empty($studentMarkTerm))
        {
            ...
            for($i = 0; $i < $numberOfLessons; $i++)
            {
                ...
                if($term->getTerm() != ConstantsClass::ANNUEL_TERM)
                {
                    ...
                    {
                        case 1 :
                            $skillEvaluation1 = $this->skillRepository->findOneBy([
                                'lesson' => $lesson,
                                'sequence' => $term->getSequences()[0]
                            ]); 

                            $skillEvaluation2 = $this->skillRepository->findOneBy([
                                'lesson' => $lesson,
                                'sequence' => $term->getSequences()[1]
                            ]); 
                            break;
                ...
                if($skillEvaluation1)
                {
                    $lessonMarkAndSkill['skillEvaluation1'] = $skillEvaluation1->getSkill();

                }
                else
                {
                    $lessonMarkAndSkill['skillEvaluation1'] = "//";
                }

                if($skillEvaluation2)
                {
                    $lessonMarkAndSkill['skillEvaluation2'] = $skillEvaluation2->getSkill();

                }
                else
                {
                    $lessonMarkAndSkill['skillEvaluation2'] = "//";
                }
    ...
    public function getClassroomProfile(array $rankedStudents): ClassroomProfile
    {
        ...
        if($rankedStudents)
        {
            $classroomProfile->setClassroomAverage($this->getClassroomAverage($rankedStudents))
                ...
                ->setNumberAverage($numberAverage)
                ; 
        }
    ...
    public function calculateTrimesterNote($notes)
    {
        $note1 = $notes[0];
        $note2 = $notes[1];

        // Si une des notes est égale à 0.1, ignorer cette note
        if ($note1 == 0.1 && $note2 != 0.1) {
            return $note2;
        } elseif ($note2 == 0.1 && $note1 != 0.1) {
            return $note1;
        } elseif ($note1 != 0.1 && $note2 != 0.1) {
            return ($note1 + $note2) / 2;
        }

        return null; // Si les deux notes sont égales à 0.1, ne pas les prendre en compte
    }
     ...
     * @return ReportBody
     */
    public function getStudentReportBody(array $studentMarkSequence1, 
                                        array $studentMarkSequence2, 
                                        array $studentMarkTerm, 
                                        array $rankedStudents, 
                                        int $index, 
                                        int $numberOfLessons, 
        ...
        if(count($studentMarkTerm) == ($numberOfLessons*$numberOfStudents))
        {
            ...
            {   
                ...
                if($term->getTerm() != 0)
                {
                    ...
                    $maxMoyenne = PHP_FLOAT_MIN;
                    
                    foreach ($rankedStudents[0]['student']->getClassroom()->getStudents() as $student)
                    {
                        ...
                        {
                            if($evaluation->getLesson()->getSubject()->getSubject() == $studentMarkSequence1[$j]->getLesson()->getSubject()->getSubject())
                            {
                                switch ($term->getTerm()) 
                                {
                                    case 1:
                                        // Récupérer les notes de l'évaluation
                                        if($evaluation->getSequence()->getSequence() == 1 || $evaluation->getSequence()->getSequence() == 2)
                                        $notes[] = $evaluation->getMark();

                                        break;

                                    case 2:
                                        // Récupérer les notes de l'évaluation
                                        if($evaluation->getSequence()->getSequence() == 3 || $evaluation->getSequence()->getSequence() == 4)
                                        $notes[] = $evaluation->getMark();
                                        break;

                                    case 3:
                                        // Récupérer les notes de l'évaluation
                                        if($evaluation->getSequence()->getSequence() == 5 || $evaluation->getSequence()->getSequence() == 6)
                                        $notes[] = $evaluation->getMark();
                                        break;
                        ...
                        #je complète avec les 0.1 si nécessaire
                        $notes = array_pad($notes, 2, 0.1);

                        #je calcul la moyenne annuelle
                        $totalNotes = 0;
                        $nombreNoteValide = 0;
                        ...
                        if($nombreNoteValide > 0)
                        {
                            $moyenneAnnuelle = $totalNotes / $nombreNoteValide;

                            if($moyenneAnnuelle < $minMoyenne)
                            {
                                $minMoyenne = $moyenneAnnuelle;
                            }

                            if($moyenneAnnuelle > $maxMoyenne)
                            {
                                $maxMoyenne = $moyenneAnnuelle;
                            }
                        }
                    }
                }
                ...
                $reportRow->setSubject($studentMarkSequence1[$j]->getLesson()->getSubject()->getSubject())
                    ...
                    ->setAppreciationEn($this->generalService->getApcAppreciationEn($mark))
                    ->setCote($this->generalService->getCote($mark));

                    // if($term->getTerm() != 0)
                    // {
                    //     $reportRow->setMinNote($minMaxNotes[0]['minNote'] ? $minMaxNotes[0]['minNote'] : 0)
                    //     ->setMaxNote($minMaxNotes[0]['maxNote'] ? $minMaxNotes[0]['maxNote'] : 1);
                    // }
                    // else
                    // {
        ...
        foreach($allStudentReports as $report)
        {
            ...
            if($report->getReportHeader()->getStudent()->isSolvable() == 1)
            {
                ...
                if ($subSystem->getSubSystem() == constantsClass::FRANCOPHONE) 
                {
                    ...
                    $pdf->Cell(50, $cellHeaderStudentHeight, $student->getBirthday()->format('d/m/Y').''.utf8_decode(' à ').''.utf8_decode($student->getBirthplace()), 0, 0, 'L');

                    $pdf->SetFont('Times', '', $fontSize-1);
                    $pdf->Cell(13, $cellHeaderStudentHeight, 'Sexe : ', 0, 0, 'L');

                    $pdf->SetFont('Times', 'B', $fontSize-1);
                    $pdf->Cell(10, $cellHeaderStudentHeight, $student->getSex()->getSex(), 0, 1, 'L');
                    ...
                    // $pdf->Cell(10, $cellHeaderStudentHeight, $student->getRepeater()->getRepeater(), 0, 1, 'L');
                    ...
                    $pdf->Cell(35, $cellHeaderStudentHeight, utf8_decode('Garçons : ').$numberOfBoys.'    Filles : '.$numberOfGirls, 0, 0, 'L');
                ...
                else 
                {
                    ...
                    $pdf->Cell(35, $cellHeaderStudentHeight, utf8_decode('Boys : ').$numberOfBoys.'    Girls : '.$numberOfGirls, 0, 0, 'L');
                ...
                #photo de l'élève sur la tableau d'honneur
                if($student->getPhoto())
                {
                ...
                }else
                {
                    ...
                    {
                        $pdf->Image('images/students/fille.jpg', 15, 52, 25, 25);
                    }
                    else
                    {
                ...
                // Chemin vers le dossier où les codes QR sont enregistrés
                $qrDirectory = $this->parameterBag->get('kernel.project_dir') . '/public/images/qrcode/';

                // Nom du fichier QR (par exemple, basé sur un identifiant unique)
                $qrFileName = $student->getQrCode();
                ...
                if($subSystem->getSubSystem() == constantsClass::FRANCOPHONE)
                {
                    ...
                    if($term->getTerm() != ConstantsClass::ANNUEL_TERM) 
                    {
                        $pdf->Cell($cellSkillWidth-7, $cellTableHeaderHeight, utf8_decode('Compétences visées'), 1, 0, 'C', true);
                    ...
                    {
                        $pdf->Cell($cellEvaluationWidth-4, $cellTableHeaderHeight, $e1, 1, 0, 'C', true);
                    }

                    if($term->getTerm() == ConstantsClass::ANNUEL_TERM)
                    {
                        $pdf->Cell($cellEvaluationWidth-4, $cellTableHeaderHeight, $e2, 1, 0, 'C', true);
                        $pdf->Cell($cellEvaluationWidth-4, $cellTableHeaderHeight, $e3, 1, 0, 'C', true);
                    ...
                    if($term->getTerm() != ConstantsClass::ANNUEL_TERM)
                    {
                        $pdf->Cell($cellAverageWidth-3, $cellTableHeaderHeight, 'Moy', 1, 0, 'C', true);
                        $pdf->Cell($cellCoefficientWidth-2, $cellTableHeaderHeight, 'Coef', 1, 0, 'C', true);
                        $pdf->SetFont('Times', 'B', $fontSize - 3);
                        $pdf->Cell($cellTotalWidth, $cellTableHeaderHeight, 'M x Coef', 1, 0, 'C', true);
                    }
                    else
                    {
                        $pdf->Cell($cellAverageWidth-5, $cellTableHeaderHeight, 'Moy', 1, 0, 'C', true);
                        $pdf->Cell($cellCoefficientWidth-7, $cellTableHeaderHeight, 'Coef', 1, 0, 'C', true);
                        $pdf->SetFont('Times', 'B', $fontSize - 3);
                        $pdf->Cell($cellTotalWidth-5, $cellTableHeaderHeight, 'M x Coef', 1, 0, 'C', true);
                    }
                    
                    $pdf->SetFont('Times', 'B', $fontSize);
                    if($term->getTerm() != ConstantsClass::ANNUEL_TERM)
                    {
                        $pdf->Cell($cellRankWidth, $cellTableHeaderHeight, 'Cote', 1, 0, 'C', true);
                    }
                    else
                    {
                        $pdf->Cell($cellRankWidth-5, $cellTableHeaderHeight, 'Cote', 1, 0, 'C', true);
                    }
                    $pdf->SetFont('Times', 'B', $fontSize - 3);
                    $pdf->Cell($cellRankWidth, $cellTableHeaderHeight, 'Min-Max', 1, 0, 'C', true);
                    $pdf->SetFont('Times', 'B', $fontSize);
                ...
                }else
                {
                    ...
                    if($term->getTerm() != ConstantsClass::ANNUEL_TERM) 
                    {
                        $pdf->Cell($cellSkillWidth-7, $cellTableHeaderHeight, utf8_decode('Target skill'), 1, 0, 'C', true);
                    ...
                    if($term->getTerm() != ConstantsClass::ANNUEL_TERM)
                    {
                        $pdf->Cell($cellAverageWidth-3, $cellTableHeaderHeight, 'Avg', 1, 0, 'C', true);
                        $pdf->Cell($cellCoefficientWidth-2, $cellTableHeaderHeight, 'Coef', 1, 0, 'C', true);
                        $pdf->SetFont('Times', 'B', $fontSize - 3);
                        $pdf->Cell($cellTotalWidth, $cellTableHeaderHeight, 'Avg x Coef', 1, 0, 'C', true);
                    }
                    else
                    {
                        $pdf->Cell($cellAverageWidth-5, $cellTableHeaderHeight, 'Avg', 1, 0, 'C', true);
                        $pdf->Cell($cellCoefficientWidth-7, $cellTableHeaderHeight, 'Coef', 1, 0, 'C', true);
                        $pdf->SetFont('Times', 'B', $fontSize - 3);
                        $pdf->Cell($cellTotalWidth-5, $cellTableHeaderHeight, 'Avg x Coef', 1, 0, 'C', true);
                    }
                ...
                $pdf->SetFont('Times', '', $fontSize-1);
                $pdf->Cell($w11, $cellTableBodyHeight, 'Cote', 'LB', 0, 'L');

                $pdf->SetFont('Times', 'B', $fontSize+4);
                
                $pdf->Cell($w21, $cellTableBodyHeight, $this->generalService->getCote((int)$moyenne), 'BR', 1, 'C');
                ...
                if($subSystem->getSubSystem() == constantsClass::FRANCOPHONE)
                {
                    $pdf->Cell($w12, $cellTableBodyHeight, 'Nbre de moyenne', 'LB', 0, 'L');
                }
                else
                {
                    $pdf->Cell($w12, $cellTableBodyHeight, 'Number of average', 'LB', 0, 'L');
                }

                $pdf->SetFont('Times', 'B', $fontSize-1);
                $pdf->Cell($w22, $cellTableBodyHeight, $classroomProfile->getNumberAverage(), 'BR', 1, 'R');
                ...
                $pdf->SetFont('Times', 'B', $fontSize-1);
                // dd($reportFooter->getStudentResult()->getMoyenne());
                if($subSystem->getSubSystem() == constantsClass::FRANCOPHONE)
                {
                ...
                if ($subSystem->getSubSystem() == constantsClass::FRANCOPHONE) 
                {
                    ...
                    switch ($workAppreciation->getAppreciation()) 
                    {
                        ...
                        case ConstantsClass::CTBA:
                            $pdf->Cell(21, $cellTableBodyHeight, utf8_decode($workAppreciation->getAppreciation()), 'TRB', 0, 'C');
                            
                            break;

                        case ConstantsClass::CBA:
                            $pdf->Cell(21, $cellTableBodyHeight, utf8_decode($workAppreciation->getAppreciation()), 'TRB', 0, 'C');
                            
                            break;
                        
                        case ConstantsClass::CA_FR:
                            $pdf->Cell(21, $cellTableBodyHeight, utf8_decode($workAppreciation->getAppreciation()), 'TRB', 0, 'C');
                            
                            break;

                        case ConstantsClass::CMA:
                            $pdf->Cell(21, $cellTableBodyHeight, utf8_decode($workAppreciation->getAppreciation()), 'TRB', 0, 'C');
                            
                            break;

                        case ConstantsClass::CNA_FR:
                            $pdf->Cell(21, $cellTableBodyHeight, utf8_decode($workAppreciation->getAppreciation()), 'TRB', 0, 'C');
                ...
                else
                {
                    switch ($workAppreciation->getAppreciation()) 
                    {
                        ...
                        case ConstantsClass::CVWA:
                            $pdf->Cell(21, $cellTableBodyHeight, utf8_decode($workAppreciation->getAppreciation()), 'TRB', 0, 'C');
                            
                            break;

                        case ConstantsClass::CWA:
                            $pdf->Cell(21, $cellTableBodyHeight, utf8_decode($workAppreciation->getAppreciation()), 'TRB', 0, 'C');
                            
                            break;
                        
                        case ConstantsClass::CA_EN:
                            $pdf->Cell(21, $cellTableBodyHeight, utf8_decode($workAppreciation->getAppreciation()), 'TRB', 0, 'C');
                            
                            break;

                        case ConstantsClass::CAA:
                            $pdf->Cell(21, $cellTableBodyHeight, utf8_decode($workAppreciation->getAppreciation()), 'TRB', 0, 'C');
                            
                            break;

                        case ConstantsClass::CNA_EN:
                            $pdf->Cell(21, $cellTableBodyHeight, utf8_decode($workAppreciation->getAppreciation()), 'TRB', 0, 'C');
    ...
    public function displayMarkGroup(array $rowsGroup, StudentResult $reportResult,  PDF $pdf, int $fontSize, int $cellSubjectWidth, int $cellTableBodyHeight, Term $term, int $cellSkillWidth, int $cellEvaluationWidth, int $cellAverageWidth, int $cellCoefficientWidth, int $cellTotalWidth, int $cellRankWidth, int $cellAppreciationWidth, int $cellRecapNameWidth, int $cellRecapTotalCoefficientWidth, int $cellRecapTotalMarkWidth, int $cellRecapAverageWidth, int $cellRecapRankWidth, Student $student, SubSystem $subSystem, SchoolYear $schoolYear): PDF
    {
        ...
        foreach($rowsGroup as $reportRow)
        {
            ...
            if($term->getTerm() != ConstantsClass::ANNUEL_TERM)
            {
                ...
                // dd($reportRow);
                // if(strlen($skillEvaluation1) <= 53)
                // {   
                //     if($skillEvaluation1)
                //     {
                //         $pdf->Cell($cellSkillWidth-7, $cellTableBodyHeight/2, $skillEvaluation1, 1, 0, 'L');
                //     }
                //     else
                //     {
                //         $pdf->Cell($cellSkillWidth-7, $cellTableBodyHeight/2, $skill, 1, 0, 'L');
                //     }
                ...
                // {
                    ...
                    $y = $pdf->GetY();
                    $pdf->Ln();
                    $x1 = $pdf->SetXY($x-49, $y + ($cellTableBodyHeight/2));
            ...
            if($term->getTerm() != ConstantsClass::ANNUEL_TERM)
            {
                $pdf->Cell($cellEvaluationWidth, $cellTableBodyHeight/2, $this->generalService->formatMark($reportRow->getEvaluation1()), 1, 0, 'C');
            }
            else
            {
                $pdf->Cell($cellEvaluationWidth-4, $cellTableBodyHeight, $this->generalService->formatMark($reportRow->getEvaluation1()), 1, 0, 'C');
            ...
            {
                $pdf->Ln();
                $pdf->SetXY($cellSubjectWidth + $cellSkillWidth + $cellEvaluationWidth - 2, $y + ($cellTableBodyHeight/2));

                $pdf->Cell($cellEvaluationWidth, $cellTableBodyHeight/2, $this->generalService->formatMark($reportRow->getEvaluation2()), 1, 0, 'C');
            }
            else
            {
                $pdf->Cell($cellEvaluationWidth-4, $cellTableBodyHeight, $this->generalService->formatMark($reportRow->getEvaluation2()), 1, 0, 'C');
            }
            
            $pdf->SetFont('Times', '', $fontSize-1);
            if($term->getTerm() == ConstantsClass::ANNUEL_TERM)
            {
                $pdf->Cell($cellEvaluationWidth-4, $cellTableBodyHeight, $this->generalService->formatMark($reportRow->getEvaluation3()), 1, 0, 'C');
            }
            ...
            if($term->getTerm() != ConstantsClass::ANNUEL_TERM)
            {
                $pdf->SetXY(($cellSubjectWidth + $cellSkillWidth + $cellEvaluationWidth - 2) + $cellAverageWidth - 5, ($y + ($cellTableBodyHeight/2)) - ($cellTableBodyHeight/2)*2);
            }
            
            if($term->getTerm() != ConstantsClass::ANNUEL_TERM)
            {
                $pdf->Cell($cellAverageWidth-3, $cellTableBodyHeight, $this->generalService->formatMark($reportRow->getMoyenne()), 1, 0, 'C', true);
                $pdf->SetFont('Times', '', $fontSize);
                $pdf->Cell($cellCoefficientWidth-2, $cellTableBodyHeight, $this->formatCoefficient($reportRow->getCoefficient()), 1, 0, 'C');
            }
            else
            {
                $pdf->Cell($cellAverageWidth-5, $cellTableBodyHeight, $this->generalService->formatMark($reportRow->getMoyenne()), 1, 0, 'C', true);

                $pdf->SetFont('Times', '', $fontSize);
                $pdf->Cell($cellCoefficientWidth-7, $cellTableBodyHeight, $this->formatCoefficient($reportRow->getCoefficient()), 1, 0, 'C');
            ...
            if($reportRow->getMoyenne() != ConstantsClass::UNRANKED_MARK)
            {
                ...
                if($term->getTerm() != ConstantsClass::ANNUEL_TERM)
                {
                    ...
                    $pdf->Cell($cellRankWidth, $cellTableBodyHeight, utf8_decode($reportRow->getCote()), 1, 0, 'C');
                }
                else
                {
