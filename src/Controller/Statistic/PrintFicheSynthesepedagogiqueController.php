...
#[Route("/statistic")]
class PrintFicheSynthesepedagogiqueController extends AbstractController
{
    ...
    #[Route("/print-fiche-synthese-pedagogique", name:"print_fiche_synthese_pedagogique")]
    public function printFicheSynthesepedagogique(Request $request): Response
    {
        ...
        // On construit la partie des taux de réussite par classe 
        $classroomStatisticSlipPerSubjects = $this->statisticService->getStatisticSlipPerSubject($studentMarkTermsSubjects);
        
        $pdf = $this->statisticService->printPedagogicalSummarySheet($classroomStatisticSlipPerSubjects, $firstPeriodLetter, $idP, $school, $schoolYear, $lessons, $subSystem);

        switch ($firstPeriodLetter) {
            case 's':
                if ($subSystem->getId() == 1 ) 
                {
                    return new Response($pdf->Output(utf8_decode("Educational summary of the evaluation ".$idP), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                } else {
                    return new Response($pdf->Output(utf8_decode("Fiche de synthèse pédagogique de l'évaluation ".$idP), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                }
                break;
            ...
            case 't':
                if ($subSystem->getId() == 1 ) 
                {
                    return new Response($pdf->Output(utf8_decode("Educational summary of the term ".$idP), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                } else {
                    return new Response($pdf->Output(utf8_decode("Fiche de synthèse pédagogique du trimestre ".$idP), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                }
                break;
            ...
            default:
                if ($subSystem->getId() == 1 ) 
                {
                    return new Response($pdf->Output(utf8_decode("Annual educational summary sheet"), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                } else {
                    return new Response($pdf->Output(utf8_decode("Fiche de synthèse pédagogique annuel"), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                }
                break;
        }
    }
