...
class SendTranscriptSequenceController extends AbstractController
{
    ...
    #[Route("/send-transcript-sequence-student/{slugStudent}/{sequenceId}", name:"send_transcript_sequence_student")]
    #[Route("/send-transcript-sequence-classroom/{slugClassroom}/{sequenceId}", name:"send_transcript_sequence_classroom")]
    public function sendSequenceTranscript(Request $request, 
    MailerInterface $mailer, TransportInterface $transport,  string $slugStudent = null, string $slugClassroom = null, int $sequenceId = 0): Response
    {
        ...
        if ($sequenceId && !$slugClassroom) 
        {
            ...
            {
                $filePath = $this->getParameter('kernel.project_dir') . '/public/releves/Transcript of ' . $student->getFullName() .' evaluation '. $sequence->getSequence().'.pdf';
            }
            else
            {
                $filePath = $this->getParameter('kernel.project_dir') . '/public/releves/Relevé de notes de ' . $student->getFullName().' évaluation '. $sequence->getSequence().'.pdf';
            }
            ...
            try 
            {
                ...
                $envoie = 1;
                $this->printTranscriptService->printTranscriptStudentSequence($subSystem, $schoolYear, $school, $student->getClassroom(), $releves, $student, $studentName, $term, $sequence, $envoie, $filePath);
            } 
            catch (\Exception $e) 
            {
            ...
            $email = (new TemplatedEmail())
                ...
                ->attachFromPath($filePath, 'Relevé de note de ' . $student->getFullName() .' évaluation '. $sequence->getSequence().'.pdf', 'application/pdf');
        ...
        if ($request->request->has('slugClassroom') && $request->request->has('sequenceId')) 
        {
            ...
            {
                $pdf = $this->printTranscriptService->sendTranscriptStudentSequence($subSystem, $schoolYear, $school, $classroom, $releves, $student, $releves['studentName'], $term, $sequence);
            
                // Génération du PDF
                if ($subSystem->getId() == 1 ) 
                {
                    $filePath = $this->getParameter('kernel.project_dir') . '/public/releves/Transcript of  - ' . $releves['studentName'] .' - evaluation '. $sequence->getSequence().'.pdf';
                }
                else
                {
                    $filePath = $this->getParameter('kernel.project_dir') . '/public/releves/Relevé de notes de - ' . $releves['studentName'].' - evaluation '. $sequence->getSequence().'.pdf';
                }
                ...
                try 
                {
                    // Générer et enregistrer le fichier PDF
                    $envoie = 1;
                    $this->printTranscriptService->sendTranscriptStudentSequence($subSystem, $schoolYear, $school, $classroom, $releves, $student, $studentName, $term, $sequence, $filePath);
                } 
                catch (\Exception $e) 
                {
                    $this->addFlash('danger', $this->translator->trans("Transcript generation error !"));
                ...
                $email = (new TemplatedEmail())
                    ...
                    ->attachFromPath($filePath, 'Relevé de notes de ' . $releves['studentName'] .' - évaluation '. $sequence->getSequence().'.pdf', 'application/pdf');
