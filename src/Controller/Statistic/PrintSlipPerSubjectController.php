...
#[Route("/statistic")]
class PrintSlipPerSubjectController extends AbstractController
{
    ...
    #[Route("/printSlipPerSubject/{resume<[0-1]{1}>}", name:"statistic_printSlipPerSubject")]
    public function printSlipPerSubject(Request $request, int $resume = 0): Response
    {
        ...
        $pdf = $this->statisticService->printStatisticSlipPerSubject($classroomStatisticSlipPerSubjects, $firstPeriodLetter, $idP, $school, $schoolYear, $resume);
        ...
        switch ($firstPeriodLetter) 
        {
            case 's':
                ...
                {
                    return new Response($pdf->Output(utf8_decode("Slip per subject of evaluation ".$idP ), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                } else {
                    return new Response($pdf->Output(utf8_decode("Statistiques par matière de l'évaluation ".$idP), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                }
                break;
            ...
            case 't':
                if ($subSystem->getId() == 1 ) 
                {
                    return new Response($pdf->Output(utf8_decode("Slip per subject of term ".$idP ), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                } else {
                    return new Response($pdf->Output(utf8_decode("Statistiques par matière du trimestre ".$idP), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                }
                break;
