...
#[Route("/statistic")]
class PrintRateOfSuccessPerClassController extends AbstractController
{
    ...
    #[Route("/printRateOfSuccessPerClass", name:"statistic_printRateOfSuccessPerClass")]
    public function printRateOfSuccessPerClass(Request $request): Response
    {
        ...
        $pdf = $this->statisticService->printRateOfSuccessPerClass($rateOfSuccessPerClass, $schoolYear, $firstPeriodLetter, $idP, $school, $subSystem);
        ...
        switch ($firstPeriodLetter) 
        {
            case 's':
                ...
                {
                    return new Response($pdf->Output(utf8_decode("Rate of success per classe of evaluation ".$idP ), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                } else {
                    return new Response($pdf->Output(utf8_decode("Taux de réussite par classe de l'évaluation ".$idP), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                }
                break;
            ...
            case 't':
                if ($subSystem->getId() == 1 ) 
                {
                    return new Response($pdf->Output(utf8_decode("Rate of success per classe of term ".$idP ), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                } else {
                    return new Response($pdf->Output(utf8_decode("Taux de réussite par classe du trimestre ".$idP), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                }
                break;
            ...
            default:
                if ($subSystem->getId() == 1 ) 
                {
                    return new Response($pdf->Output(utf8_decode("Annual rate of success per classe" ), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                } else {
                    return new Response($pdf->Output(utf8_decode("Taux de réussite annuel par classe"), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                }
                break;
        }
