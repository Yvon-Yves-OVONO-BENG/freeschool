...
class GeneralService 
{
    ...
    public function getApoAppreciation(float $mark): string
    {
        ...
        if($mark != ConstantsClass::UNRANKED_MARK)
        {
            ...
            {
                if($mark >= 0 && $mark < 7)
                    return ConstantsClass::FAIBLE;
                    
                elseif($mark >= 7 && $mark < 9)
                    return ConstantsClass::INSUFFISANT;
                    
                elseif($mark >= 9 && $mark < 10)
                    return ConstantsClass::MEDIOCRE;

                elseif($mark >= 10 && $mark < 12)
                    return ConstantsClass::PASSABLE;
                    
                elseif($mark >= 12 && $mark < 14)
                    return ConstantsClass::ASSEZ_BIEN;
                    
                elseif($mark >= 14 && $mark < 16)

                    return ConstantsClass::BIEN;
                    
                elseif($mark >= 16 && $mark < 18)

                    return ConstantsClass::TRES_BIEN;
                    
                elseif($mark >= 18 && $mark < 20)
                    
                    return ConstantsClass::EXCELLENT;

                elseif($mark == 20)
                    return ConstantsClass::PARFAIT;
            }else
            {
                if($mark >= 0 && $mark < 7)
                return ConstantsClass::WEAK;
                
            elseif($mark >= 7 && $mark < 9)
                return ConstantsClass::INSUFFICIENT;
                
            elseif($mark >= 9 && $mark < 10)
                return ConstantsClass::POOR;

            elseif($mark >= 10 && $mark < 12)
                return ConstantsClass::FAIR;
                
            elseif($mark >= 12 && $mark < 14)
                return ConstantsClass::PRETTY_GOOD;
                
            elseif($mark >= 14 && $mark < 16)

                return ConstantsClass::GOOD;
                
            elseif($mark >= 16 && $mark < 18)

                return ConstantsClass::ALRIGHT;
                
            elseif($mark >= 18 && $mark < 20)
                
                return ConstantsClass::EXCELENT;

            elseif($mark == 20)
                return ConstantsClass::PERFECT;
            }
    ...
    public function getAdministrativeHeader(School $school, PDF $pdf, float $cellHeaderHeight, int $fontSize, SchoolYear $schoolYear): PDF
    {
        ...
        $pdf->Cell($wHeader, $cellHeaderHeight, utf8_decode($school->getFrenchCountry()) , 0, 0, 'C');
            ...
            $pdf->Image('images/school/'.$school->getLogo(), 90, 7, -200);
