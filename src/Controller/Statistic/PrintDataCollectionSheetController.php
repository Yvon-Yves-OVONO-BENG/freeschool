...
#[Route("/statistic")]
class PrintDataCollectionSheetController extends AbstractController
{
    ...
    #[Route("/print-data-collection-sheet", name:"print_data_collection_sheet")]
    public function printDataCollectionSheet(Request $request): Response
    {
        ...
        $pdf = $this->statisticService->printDataCollection($classroomStatisticSlipPerSubjects, $firstPeriodLetter, $idP, $school, $schoolYear, $lessons, $subSystem);
        ...
        switch ($firstPeriodLetter) 
        {
            case 's':
                ...
                {
                    return new Response($pdf->Output(utf8_decode("Data collection of evaluation ".$idP), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                } 
                else 
                {
                    return new Response($pdf->Output(utf8_decode("Fiche de collecte de données de l'évaluation ".$idP), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                }
                break;
            ...
            case 't':
                if ($subSystem->getId() == 1 ) 
                {
                    return new Response($pdf->Output(utf8_decode("Data collection of term ".$idP), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                } 
                else 
                {
                    return new Response($pdf->Output(utf8_decode("Fiche de collecte de données du trimestre ".$idP), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                }
                break;
            ...
            default:
                if ($subSystem->getId() == 1 ) 
                {
                    return new Response($pdf->Output(utf8_decode("Data collection annual "), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                } 
                else 
                {
                    return new Response($pdf->Output(utf8_decode("Fiche de collecte de données annuel"), "I"), 200, ['Content-Type' => 'application/pdf']) ;
                }
                break;
        }
    }
